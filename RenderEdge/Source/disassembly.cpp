...
namespace base { namespace hook { namespace detail {
	...
	enum OP_FLAG
	{
		...
		OP_DATA_I16      = 0x04,
		OP_DATA_I32      = 0x08,
		OP_DATA_PRE66_67 = 0x10,
		OP_WORD          = 0x20,
		OP_REL32         = 0x40,
	};

	uint8_t opcode_flags[256] =
	{
		OP_MODRM,                      // 00
		OP_MODRM,                      // 01
		OP_MODRM,                      // 02
	...
	size_t next_opcode(uintptr_t code, uint8_t** opcode_ptr)
	{
		...
		bool pfx67 = false;
		bool sib_present;
		uint8_t imod, irm, ireg;
		uint8_t offset_size = 0;
		uint8_t add;
		uint8_t opcode;

		while ((*ptr == 0x2E) || (*ptr == 0x3E) || (*ptr == 0x36) ||
			   (*ptr == 0x26) || (*ptr == 0x64) || (*ptr == 0x65) ||
			   (*ptr == 0xF0) || (*ptr == 0xF2) || (*ptr == 0xF3) ||
			   (*ptr == 0x66) || (*ptr == 0x67))
		{
			if (*ptr == 0x66) pfx66 = true;
			if (*ptr == 0x67) pfx67 = true;
			ptr++;
			if (ptr > (uint8_t*)code + 16) return 0;
		}
		...
		{
			ptr++;
			flags = opcode_flags_ext[*ptr];
		} 
		else
		{
			flags = opcode_flags[opcode];
		...
		if (flags & OP_MODRM)
		{
			...
			{
			case 0:
				if ( pfx67 && (irm == 6)) offset_size = 2;
				if (!pfx67 && (irm == 5)) offset_size = 4;
				break;
			case 1: offset_size = 1;
				break;
			case 2: if (pfx67) offset_size = 2; else offset_size = 4;
				break;
			case 3: sib_present = false;
			}
			if (sib_present)
			{
				if (((*ptr & 7) == 5) && ( (!imod) || (imod == 2) )) offset_size = 4;
				ptr++;
			}
			ptr += offset_size;
		}

		if (flags & OP_DATA_I8)  ptr++;
		if (flags & OP_DATA_I16) ptr += 2;
		if (flags & OP_DATA_I32) ptr += 4;
		if (pfx66) add = 2; else add = 4;
		if (flags & OP_DATA_PRE66_67) ptr += add;
		return (uintptr_t)ptr - code;
	}
}}}
